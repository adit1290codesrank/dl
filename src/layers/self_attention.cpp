...
Tensor SelfAttention::forward(const Tensor& input)
{
    ...
    Tensor Qh(std::vector<int>{N*H,T,head_dim});
    Tensor Kh(std::vector<int>{N*H,T,head_dim});
    Tensor Vh(std::vector<int>{N*H,T,head_dim});
    ...
    Tensor scores(std::vector<int>{N*H,T,T});
    ...
    Tensor attn(std::vector<int>{N*H,T,T});
    ...
    // 7. Context = Attention @ V -> (N*H, T, dk)
    Tensor context(std::vector<int>{N*H,T,head_dim});
...
Tensor SelfAttention::backward(const Tensor& dY,float lr)
{
    ...
    Tensor dContext(std::vector<int>{N*H,T,head_dim});
