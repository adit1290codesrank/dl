...
Tensor BatchNorm1D::forward(const Tensor& input)
{
    ...
    if(is_training)
    {
        ...
        batchnorm_cuda(input,this->cached_mean,this->cached_var,g,b,output,output,e);
        update_running_stats_cuda(this->running_mean,this->running_var,this->cached_mean,this->cached_var,m);
    }
    else batchnorm_cuda(input,this->running_mean,this->running_var,g,b,output,output,e);
    return output;
}
...
Tensor BatchNorm1D::backward(const Tensor& grad,float learning_rate)
{
    ...
    batchnorm_forward_cuda(grad,this->cached_mean,dg,db);
    batchnorm_backward_cuda(grad,this->cached_mean,this->cached_var,g,dg,db,dX,e);
...
void BatchNorm1D::load(std::ifstream& is)
{
    ...
    this->g=Tensor::upload(h_g,this->features,1);
    this->b=Tensor::upload(h_b,this->features,1);
